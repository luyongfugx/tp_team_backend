...
import { attachPhotosToFeed, feedInclude, feedPrisma, mapFeed, parseFeedPhotoIDs, parseFeedType, parseProjectID, validatePhotoScopes, validateProjectScope } from "@/app/api/_utils/feed"
...
export async function POST(req: Request) {
  try {
    ...
    if (!title && !content && photoIDs.length === 0 && !payload) return bad("请输入动态内容")
    if (!(await validatePhotoScopes(groupID, photoIDs, projectID))) return bad("照片不存在")
    if (feedID) {
      const existingFeed = await feedPrisma.teamFeed.findFirst({
    ...
    const feedWithInclude = await feedPrisma.teamFeed.findFirst({
      where: { groupID, feedID: feed.feedID, deletedAt: null },
      include: feedInclude,
    })

    return ok({ feedInfo: jsonSafe(mapFeed(feedWithInclude || feed, user.id)) })
  } catch (err) {
