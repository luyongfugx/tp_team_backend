...
export async function POST(req: Request) {
  try {
    ...
    if (existing) return ok({ likeID: existing.likeID, liked: true, alreadyLiked: true })
    ...
    const like = await prisma.$transaction(async (tx) => {
      ...
      const created = await delegate.teamFeedLike.create({
        data: { groupID, feedID, userID: user.id },
      })
    ...
    return ok({ likeID: like.likeID, liked: true, alreadyLiked: false })
  } catch (err) {
